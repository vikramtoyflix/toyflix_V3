...
-- Update the handle_new_user function to handle existing phone numbers
...
BEGIN
  ...
  IF new.raw_user_meta_data->>'phone' IS NOT NULL THEN
    ...
    ELSE
      -- Phone doesn't exist, create new profile
...
-- This handles cases where profiles were created but auth user creation failed
DELETE FROM public.profiles 
WHERE phone IS NOT NULL 
  AND phone_verified = true 
  AND id NOT IN (
    SELECT id FROM auth.users WHERE phone IS NOT NULL
  );
