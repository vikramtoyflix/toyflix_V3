-- ========================================
-- Subscription Cycle Tracking Schema Migration (FIXED)
...
-- Update existing subscriptions to have proper cycle tracking (FIXED)
...
SET 
    ...
    cycle_number = GREATEST(1, DATE_PART('day', AGE(CURRENT_DATE::timestamp, start_date::timestamp))::INTEGER / 30 + 1),
    total_cycles_completed = GREATEST(0, DATE_PART('day', AGE(CURRENT_DATE::timestamp, start_date::timestamp))::INTEGER / 30),
...
-- ========================================
-- STEP 4: Create subscription timeline views (FIXED)
-- ========================================

-- View for current cycle status (FIXED with DATE_PART and AGE)
...
SELECT 
    ...
    -- Days until/since selection window (FIXED)
    CASE 
    ...
    CASE 
        WHEN s.subscription_start_date IS NOT NULL THEN
            DATE_PART('day', AGE(CURRENT_DATE::timestamp, s.subscription_start_date::timestamp))::INTEGER
        ELSE 0::INTEGER
...
SELECT 
    ...
    -- Selection window duration (FIXED)
    ...
    -- Cycle completion metrics (FIXED)
    CASE 
...
-- View for selection windows (FIXED)
