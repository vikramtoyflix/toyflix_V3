-- Enhanced User Roles and Permissions System for ToyFlix (FIXED VERSION)
...
-- ================================================================================================
-- 6. UTILITY FUNCTIONS (FIXED VERSION)
-- ================================================================================================
...
-- FIXED: Function to get user's effective permissions
...
DECLARE
    ...
    perm_value JSONB;
BEGIN
    -- Initialize effective permissions as empty object
    effective_permissions := '{}'::jsonb;
    
    -- Get all active, non-expired role permissions for the user
    FOR role_record IN 
        SELECT upr.permissions
    ...
    LOOP
        current_permissions := role_record.permissions;
        
        -- Merge permissions from this role into effective_permissions
        FOR perm_key, perm_value IN SELECT * FROM jsonb_each(current_permissions)
        LOOP
            IF effective_permissions ? perm_key THEN
                -- Merge nested permissions (simple OR logic)
                IF jsonb_typeof(perm_value) = 'object' AND jsonb_typeof(effective_permissions->perm_key) = 'object' THEN
                    effective_permissions := jsonb_set(
                        effective_permissions, 
                        ARRAY[perm_key], 
                        (effective_permissions->perm_key) || perm_value
                    );
                ELSE
                    effective_permissions := jsonb_set(effective_permissions, ARRAY[perm_key], perm_value);
                END IF;
            ELSE
                effective_permissions := jsonb_set(effective_permissions, ARRAY[perm_key], perm_value);
            END IF;
...
-- FIXED: Function to check if user has specific permission
...
BEGIN
    ...
    -- Check for "all" permission which grants everything
    IF permissions ? 'all' AND (permissions->'all')::text = 'true' THEN
        RETURN true;
    END IF;
    ...
    RETURN (current_level::text = 'true');
END;
$$;
