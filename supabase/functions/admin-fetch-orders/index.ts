...
serve(async (req) => {
  ...
  try {
    ...
    let rentalQuery = supabaseAdmin
      .from("rental_orders")
      .select("id, order_number, user_id, status, total_amount, created_at, rental_start_date, rental_end_date, order_type, payment_status, user_phone, subscription_plan, coupon_code, discount_amount, toys_data, shipping_address, razorpay_payment_id, base_amount, gst_amount");
