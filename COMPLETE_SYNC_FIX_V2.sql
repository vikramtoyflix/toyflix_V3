-- ============================================================================
-- COMPLETE TOY SYNC FIX V2 - Fixed for JSON Array Age Ranges
-- ============================================================================
-- This script fixes the sync between toys table and age-based tables
-- Now properly handles JSON arrays like ["1-2 years","2-3 years","3-4 years"]
-- ============================================================================
...
-- Step 2: Create improved sync function that handles JSON arrays
...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        -- Parse age range (handle JSON arrays, comma-separated, and simple strings)
        v_age_tables := get_applicable_age_tables(v_age_range);
...
-- Step 3: Create helper function to parse age ranges and return applicable tables
...
BEGIN
    ...
    -- Handle JSON arrays like ["1-2 years","2-3 years","3-4 years"]
    ...
    ELSE
        -- Handle comma-separated values like "2-3 years, 3-4 years"
        age_elements := string_to_array(v_age_range, ',');
    END IF;
    
    -- Process each age element
...
-- Step 5: Create improved function to manually sync ALL existing toys
...
BEGIN
    ...
    LOOP
        ...
        -- Skip toys with no applicable age tables
        IF array_length(v_age_tables, 1) IS NULL OR array_length(v_age_tables, 1) = 0 THEN
            RAISE NOTICE '⚠️ Toy % has no applicable age tables (age_range: %)', toy_record.name, toy_record.age_range;
            RETURN QUERY SELECT toy_record.id, toy_record.name, 'SKIPPED'::TEXT, 0;
            CONTINUE;
        END IF;
        
        -- Sync to applicable age tables
        FOREACH table_name IN ARRAY v_age_tables
        LOOP
            BEGIN
                -- Check if table exists
                ...
                IF v_count = 0 THEN
                    ...
                    total_updated := total_updated + 1;
                    RAISE NOTICE '✅ Inserted toy % into %', toy_record.name, table_name;
                ELSE
                    ...
                    total_updated := total_updated + 1;
                    RAISE NOTICE '✅ Updated toy % in %', toy_record.name, table_name;
                END IF;
...
-- Step 6: Test the helper function (optional)
-- SELECT get_applicable_age_tables('["1-2 years","2-3 years","3-4 years"]');

-- Step 7: Success message
DO $$
BEGIN
    RAISE NOTICE '✅ COMPLETE SYNC FIX V2 APPLIED SUCCESSFULLY!';
    RAISE NOTICE '';
    RAISE NOTICE 'What was fixed:';
    RAISE NOTICE '1. ✅ JSON arrays like ["1-2 years","2-3 years"] now properly parsed';
    RAISE NOTICE '2. ✅ Toys will be added to ALL applicable age tables';
    RAISE NOTICE '3. ✅ Comma-separated values properly handled';
    RAISE NOTICE '4. ✅ Duplicate prevention (toys wont be added twice)';
    RAISE NOTICE '';
