-- =====================================================
-- PHASE 1: CREATE UNIFIED RENTAL ORDERS TABLE
-- =====================================================
...
CREATE TABLE rental_orders (
    ...
    current_day_in_cycle INTEGER GENERATED ALWAYS AS (
        CASE 
            ...
            ELSE GREATEST(1, EXTRACT(DAY FROM (CURRENT_DATE - rental_start_date)) + 1)
        END
    ) STORED,
...
-- Grant permissions to authenticated users (adjust role names as needed)
-- GRANT SELECT, INSERT, UPDATE ON rental_orders TO authenticated;
-- GRANT USAGE ON SEQUENCE rental_orders_id_seq TO authenticated;
...
PRINT 'Phase 1 Complete: rental_orders table created with all required fields, indexes, functions, and triggers.'; 