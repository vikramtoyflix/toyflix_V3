...
// Azure Function to proxy WooCommerce database requests
module.exports = async function (context, req) {
    ...
    try {
        ...
        context.log(`WooCommerce API: ${action} - Phone: ${phone}, UserId: ${userId}, OrderId: ${orderId}`);
        ...
        context.log('Connected to WordPress database');
        ...
        switch (action) {
            case 'getUserByPhone':
                ...
                result = await getUserByPhone(connection, phone, context);
                break;
            case 'getUserOrders':
                if (!userId) throw new Error('UserId parameter is required');
                result = await getUserOrders(connection, userId, context);
                break;
            case 'getUserSubscriptions':
                if (!userId) throw new Error('UserId parameter is required');
                result = await getUserSubscriptions(connection, userId, context);
                break;
            case 'getOrderItems':
                if (!orderId) throw new Error('OrderId parameter is required');
                result = await getOrderItems(connection, orderId, context);
                break;
            default:
        ...
        context.log(`WooCommerce API: ${action} completed successfully`);
        ...
        context.log.error('WooCommerce API Error:', error.message);
...
async function getUserByPhone(connection, phone, context) {
    context.log(`Searching for user with phone: ${phone}`);
    ...
    if (rows && rows.length > 0) {
        context.log(`Found user with ID: ${rows[0].ID}`);
        return rows[0];
    } else {
        context.log(`No user found with phone: ${cleanPhone}`);
        return null;
    }
}

// Get user orders
async function getUserOrders(connection, userId, context) {
    context.log(`Fetching orders for user: ${userId}`);
    ...
    context.log(`Found ${rows?.length || 0} orders for user ${userId}`);
    return rows || [];
}

// Get user subscriptions
async function getUserSubscriptions(connection, userId, context) {
    context.log(`Fetching subscriptions for user: ${userId}`);
    ...
    context.log(`Found ${rows?.length || 0} subscriptions for user ${userId}`);
    return rows || [];
}

// Get order items
async function getOrderItems(connection, orderId, context) {
    ...
    context.log(`Found ${rows?.length || 0} items for order ${orderId}`);
    return rows || [];
} 