...
class StagingMigration {
  ...
  async updateMigrationBatch(status, stats = {}) {
    const sql = `
      ...
      SET status = $1, end_time = NOW(), 
          successful_records = COALESCE($2, successful_records),
          failed_records = COALESCE($3, failed_records)
      WHERE batch_name = $4;
    `;

    const { error } = await this.executeSQL(sql, [
      status,
      stats.successful_records || null,
      stats.failed_records || null,
      this.batchName
    ]);
  ...
  async insertUsersToStaging(users) {
    ...
    for (const user of validUsers) {
      try {
        ...
        const { data, error } = await this.executeSQL(sql, [
          user.wp_user_id, user.phone, user.email, user.first_name, user.last_name,
          user.address_line1, user.address_line2, user.city, user.state, user.zip_code,
          user.role, user.subscription_active, user.phone_verified, user.is_active,
          user.wp_created_at, user.migration_batch, user.migration_status
        ]);
  ...
  async insertOrdersToStaging(orders, userMapping) {
    ...
    for (const wpOrder of orders) {
      try {
        ...
        const { data: orderResult, error: orderError } = await this.executeSQL(orderSQL, [
          wpOrder.ID, wpOrder.customer_id, stagedUserId, 
          parseFloat(wpOrder.total || 0), this.mapOrderStatus(wpOrder.post_status), wpOrder.post_status,
          wpOrder.currency || 'INR', JSON.stringify(shippingAddress),
          wpOrder.post_date || new Date().toISOString(), 
          wpOrder.post_modified || new Date().toISOString(),
          this.batchName, 'pending'
        ]);
        ...
        if (items.length > 0 && orderResult && orderResult.length > 0) {
          ...
          for (const item of items) {
            try {
              ...
              await this.executeSQL(itemSQL, [
                item.order_item_id, orderId, item.product_id, item.order_item_name,
                parseInt(item.quantity || 1), 
                parseFloat(item.total || 0) / parseInt(item.quantity || 1),
                parseFloat(item.total || 0), this.batchName, 'pending'
              ]);
  ...
  async createProductMapping(wpItem) {
    try {
      ...
      await this.executeSQL(sql, [wpItem.product_id, wpItem.order_item_name]);
    } catch (error) {
    ...
    try {
      ...
      if (!ordersOnly) {
        ...
        if (!dryRun) {
          ...
          const { data: stagedUsers } = await this.executeSQL(`
...
async function main() {
  ...
  if (args.includes('--help')) {
    console.log(`
WooCommerce to Staging Migration Tool

Usage: node staging-migration.js [options]
...
Examples:
  node staging-migration.js --dry-run
  node staging-migration.js --users-only
  node staging-migration.js
    `);
    return;
  }
